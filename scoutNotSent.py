...
diagnostics = {}
diagnostics['mode'] = {dTimds[ix]:db.child("TempTeamInMatchDatas/"+str(dTimds[ix])+"/mode").get().val() for ix in range(len(dTimds))}
diagnostics['cycle'] = {dTimds[ix]:db.child("TempTeamInMatchDatas/"+str(dTimds[ix])+"/cycle").get().val() for ix in range(len(dTimds))}
...
if modeMode == None:
	...
	for k, v in diagnostics['mode'].items():
		if v != modeMode:
			diagnosticsMessage += "Scout " + (str(k).split("-")[1]) + ": " + str(v) + "\n"
